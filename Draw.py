class DrawCommand:
    ...
    def draw_model(self, application):
        ...
        for node in application.model.getAllNodes():
            ...
            # draw dot
            dot = 2.5
            application.frame.create_rectangle(x_corner_dist - dot, y_corner_dist - dot, x_corner_dist + dot,
                                               y_corner_dist + dot, fill="red")
            # draw label
        ...
        for node in application.model.all_leaf_nodes():
            ...
            if application.model.scale == 1.0:
                rad = length * radius
                application.frame.create_oval(x_corner_dist - rad, y_corner_dist - rad, x_corner_dist + rad,
                                              y_corner_dist + rad)
            # after optimization
            else:
        ...
        for edge in application.model.all_edges():
            ...
            application.frame.create_text(x_halfway, y_halfway,
    ...
    def draw_node(self, application, current_node, source, x, y, length, strain):
        ...
        # draw a circle
        rad = length * radius
        self.circle = application.frame.create_oval(x_corner_dist - rad, y_corner_dist - rad, x_corner_dist + rad,
                                                     y_corner_dist + rad)

        # node number and coordinates label
        self.node_label = application.frame.create_text(x_corner_dist + 10, y_corner_dist,
                                                         text="node" + str(current_node) + "\n" + "(" + str(
                                                             x) + "," + str(y) + ")")
        ...
        if source is None:
            ...
            self.dot = application.frame.create_rectangle(x_corner_dist - dot, y_corner_dist - dot,
                                                           x_corner_dist + dot, y_corner_dist + dot, fill="red")
        ...
        # draw a  line
            ...
            self.edge_label = application.frame.create_text(x_halfway, y_halfway,
                                                             text="Length = " + str(length) + "\n" + "Strain = " + str(
                                                                 strain))
