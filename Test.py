...
class TestOrigami(unittest.TestCase):
    ...
    def setUp(self): 
        ...
        self.emu.add_node_to(1, 0.5, 0.5) # creates node 2 attached to node 1
        self.emu.add_node_to(2, 0.75, 0.75) # creates node 3 attached to node 2
        self.emu.add_node_to(2, 0.25, 0.75) # creates node 4 attached to node 2
    ...
    def test_initial_guess(self):
        ...
        truth_value = np.array_equal(x0, [0.5, 0.25, 0.75, 0.75, 0.25, 0.75, 1.0])   
        self.assertTrue(truth_value)
            
    def test_construct_constraints(self):
        # create an array of the constraint functions, taken from the in-use construct_constraints function
        ...
        print("--------------")
        self.assertTrue(np.allclose(value1_from_x0, cons_array_functions[0](x0), atol=0.001))
        self.assertTrue(np.allclose(value2_from_x0, cons_array_functions[1](x0), atol=0.001))
        self.assertTrue(np.allclose(value3_from_x0, cons_array_functions[2](x0), atol=0.001))

    def test_preliminary_constraints(self):
        x0 = self.emu._scale_initial_guess()
        a = -x0[6] * 2 + self.emu.dist(0.5, 0.75, 0.25, 0.75)
        b = -x0[6] * 2 + self.emu.dist(0.5, 0.25, 0.25, 0.75)
        c = -x0[6] * 2 + self.emu.dist(0.75, 0.25, 0.75, 0.75)
        ...
        self.assertTrue(np.allclose(a, cons1_value, atol=0.0101))
        self.assertTrue(np.allclose(b, cons2_value, atol=0.0101))
        self.assertTrue(np.allclose(c, cons3_value, atol=0.0101))
