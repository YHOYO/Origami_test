...
class NodeBox(object):
    ...
    def add_node_widgets(self, entry_frame, source_frame):
        ...
        print("Formatting: 'float, float' for coordinates, length and strain can be int or float, default to 1", file=sys.stderr)
    ...
    def add_node_info(self, application, frame):
        ...
        # if there is nothing added to the model yet
        ...
        # get the node number from the radio button
            ...
            print("Error: node must have source", file=sys.stderr)
        ...
        if self.e_length.get() == "":
            length = 1.0
        else:
            length = float(self.e_length.get())
            
        if self.e_strain.get() == "":
            strain = 0.0
        else:
    ...
    # takes input string of the coordinates, returns a tuple
    def xy_entry_to_float(self):
        coord_string = self.e_xy.get()
        int_or_float = re.compile(r"\d+\.?\d*")  # Updated regex pattern to raw string
        xy = re.findall(int_or_float, coord_string)
        if len(xy) != 2:
            print("ERROR: coordinates must have x and y", file=sys.stderr)
        x = float(xy[0])
        y = float(xy[1])
        return (x, y)
